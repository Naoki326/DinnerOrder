...
/** 画像编辑的入参：各块相互独立——没传的块保持原样，传了的块整体替换 */
export interface ProfilePatch {
  /** 名字（家人自己叫的，随时可改；餐史里已存的快照不受影响——那是事实，不是外键） */
  name?: string;
  /** 头像 emoji（与名字同理） */
  emoji?: string;
  /**
   * 性别。**不只是标注**：6–17 岁小孩的份量系数按性别相差约 14%
   * （WS/T 554 表 1，6–8 岁男 0.756 / 女 0.861），新增时必填就是为了这个。
   * 既然录错了会让份量一直偏，那就得改得回来——不然只能删了重建，
   * 而那会丢掉这位家人的忌口/爱吃/餐史归属。
   *
   * 成人的折算档是 `any`（不分性别），所以改它对大人的读数无影响——
   * 但一个「录进去就永远改不了」的字段本身就是陷阱，不因当下不影响读数而放过。
   */
  gender?: 'male' | 'female';
