...
- ✅ **重刷已实测**（对生产库的副本跑 `--replace --llm`）：归一失败 **254 → 136 个名字**（305 → 142 次出现）、杂讯 26 条进报告的 `dropped` 清单，
  草稿 253 道、0 克项清零（重标 2177/2177 = 100%），家庭菜谱 18 行与逐食材克数**逐行一致**（一字未动）。
...
- **「只有调料、没有主料」的草稿有 16 道**（`汤面` 只剩水 400g、`紫菜蛋花汤` 只剩盐 2g）。
  它们**基本不是本票引入的**：数据源原文就只给了那些（`汤面` 的「面类材料」/「菜类」是分类词，
  不是食材名；`紫菜蛋花汤` 的原文只有「盐 2 克」），基线上同样有 18 道。

  **本票处理了一类、漏了一类**（2026-09-20 复核时发现，已修）：
  * ✅ **描述句当食材名**：`米粥` 的唯一食材曾经是一句说明（「水的体积是米饭的体积的」），
    包含匹配从那串字里认出「米饭」——落库成一道只有「米饭 10g」的空壳菜。
    已在 `NOISE_NAME_PATTERNS` 加「以『的』结尾」判据（中文食材名不会以助词结尾，
    字典里一个也没有，`schema-013.test.ts` 有断言钉住这条前提）。
  * ⚠ **分类词当食材名**（`面类材料`/`菜类`/`叶菜类蔬菜`）**未处理**：它们归不上字典、
    进 `unmatched` 清单，但因为**同菜里还有别的项能归上**，整道菜没被拒。
    彻底处理要动采集器的解析（识别「分类词行」），不是本票的活。

  **这 16 道进不了推荐池的理由与 0 克项不同**（`hasPendingRelabel` 只管 0 克），目前靠
  `kind` 与食材清单自然挡——若日后发现它们真上了桌，那是一个独立的小票。
  **另一条相关的事实**：`normalizeRecipe` 的注释曾写着「主料项一个都没归上就拒」，
  而代码从来没这么判过（只拒「**一个项都没归上**」）。这已收口为注释与代码一致——
  「主料没归上」**不能**判失败，那正是本票要修的缺口（`鲤鱼`/`鱼头` 以前不在字典里，
  但「糖醋鲤鱼」「红烧鱼头」是真菜，拒掉整道菜比留着缺口更糟）。
