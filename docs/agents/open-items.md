...
| 9 | **#21 M1-09 转正流程** | —（未 commit，改动在工作区） | 383 unit + 33 E2E | ✅ 实现完成，待审查 |
| 10 | #22 M1-10 留量 | — | — | 待做 |
| 11 | #23 M1-11 买菜清单 | — | — | 待做 |
...
- **B/C 视图里买菜入口仍指向占位页**（来源：#24 实施）。三视图共用底部导航（`TabBar`），
  `/grocery` 现在还是 `Placeholders.tsx` 的占位页——#23 交付后三套视图自动都有，**无需在视图层各做一套**。
...
- **C 视图（长辈小孩极简）暂无「留量 / 回顾」入口**（来源：#24 实施）。C 的第二步只放了
  「单道换 / 整套都换 / 就这么吃」——#22（留量）与 #20（餐后回顾）交付后，要在 C 里补上它们
  **同一语义的大按钮**（总纲 §2.10：三视图操作语义一致）。`SimpleView.tsx` 的模块注释已记这条。
