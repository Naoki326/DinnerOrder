...
- **新增裸名基础条目**（猪肉/鸡肉/芝麻/米饭…，带时令月份）：菜谱粗粒度语言有处落。
- **`ingredient_contains` 指针连部位**（猪排骨→猪肉、白芝麻→芝麻）：忌口自动向上传播，`avoidIngredientIds` 展开机制现成，推荐/换菜两处已接。
- **子类（品种：黑猪/野猪）不入库**：「买菜随意买」= 品种信息无需系统追踪。菜谱 A「猪肉」+ B「猪梅花肉」聚合**分列不合**（B 有硬要求，合并丢信息）。
- **44 条杂讯（「盐量 = 份数」类）在采集/解析侧过滤**，不进字典；「姜蒜」类连写名归解析侧拆分（alias 一对一救不了）。
- 完成后 `--replace --llm` 重刷草稿（重标重跑，分钟级）；新增条目必须连带时令月份与隐性忌口「含」指针。
- **否决了 parent_id 真层级**（部位/品种两维之分、忌口语义分叉、267 项重分层无权威来源；且满足 ADR 三条件得立 ADR）——日后若真要按品种追踪再议，届时立 ADR。
