...
4. **时间边界（硬规则，违反会导致整个队列挂死）**：
   - **每一个 bash 调用都必须显式传 `timeout`**（例：快速查询 15s、装包/构建 300–600s、测试 300s）。省略仅限确实瞬时返回的命令（`pwd`、`echo`）。
   - **禁止跑无界命令**：`find` 必须加 `-maxdepth`；**禁止 `find ~` / `du -sh ~` / `grep -r /` 这类全盘扫描**；不跑 `watch`、不带 `--yes` 的 `npx`、以及任何可能永不退出的 `node xxx.mjs`。
   - 长命令/服务一律**后台 + 重定向日志 + 有界轮询**；你启动的任何后台进程（dev server、测试服务）完事必须杀掉。
   - macOS 没有 `timeout` 命令；需限制时用 `curl --max-time N`、后台 + `sleep` 轮询、或有界循环。
   - 同一方向试 2–3 次没进展就停下换路。阻塞规则（同一错误连续 3 次失败 / 需产品决策而 issue 与代码库都答不了 / 撞上明显超出本工单范围的存量问题）→ 停止，`git stash`（message 带 issue 编号），报告以 `BLOCKED` 开头。
5. **不 commit**：把全部改动留在工作区（staged 或 unstaged 均可），由调度层审查后统一提交。
6. 环境的坑先读本文件，**不要再重复探测** Node/pnpm/网络/LLM 端点能力。
7. **不要为验证而搭重型基建**：项目若没有浏览器测试基建，就不要为了「跑一遍看看」去写 Playwright 脚本 / 引入 headless DOM / jsdom 等新依赖或新工具链——那类「先搭起来再验证」的尝试最耗时且最容易挂死（**已有真实事故：一个没设 timeout 的 `node debug-*.mjs` 挂了 11 分钟，拖死整张工单**）。要验证就用仓库现成的 `pnpm test` / `pnpm test:e2e`；确实需要但代价大的验证，写进报告作为「建议人工走查项」。
8. **遇到超出本工单 AC 的存量 bug**：用文字报清楚（复现条件 + 根因 + 涉及文件位置）即可，**不要修**，也不要为它搭调试基建。
